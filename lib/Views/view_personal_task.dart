...
class view_personal_task extends StatefulWidget {
  @override
  _view_personal_taskState createState() => _view_personal_taskState();
}
...
class _view_personal_taskState extends State<view_personal_task> {
  @override
  ...
  final TextEditingController personal_task_date = TextEditingController();

  String personal_task_status ;
  ...
  onstart() async {
    await personal_task_manager(isReload: true);
  }
  ...
  //simple data
  Future<List<TaskManager>> personal_task_manager({bool isReload = false}) async {
    try {
      if (isReload) {
        current_page = 0;
      }
      ...
      print(email);
      String flag = "onstart";
      url = http_root + "view_personal_task.php?page_no=$current_page&size=20&email=$email&flag=$flag";
...
//searched data
  Future<List<TaskManager>> personal_task_manager_onsearch(
    ...
    try {
      ...
      url = http_root + "view_personal_task.php?page_no=$current_search_page&size=5&word=$word&flag=$flag&email=$email";
...
//      }
  ...
  Future<List<TaskManager>> personal_task_filter_manager(
      {bool isReload = false, String per_date, String per_status}) async {
    try {
      if (isReload) {
      ...
      String flag = "onfilter";
          ...
          url = http_root + "view_personal_task.php?page_no=$current_filter_page&size=20&date=$per_date&status=$per_status&flag=$flag&email=$email";
...
//                      errorText: "",
                ...
                hint: personal_task_status == null
                    ? Text(
                  'Status',
...
//          ),
      ...
      actions: [
        GestureDetector(
          onTap: () {
   if(personal_task_status ==null && personal_task_date.text.isEmpty)
     {
     ...
     _debouncer.run(() async {
       setState(() {
         ...
         per_date=personal_task_date.text;
         per_status=personal_task_status;
       });

       personal_task_filter_manager(
         isReload: true,
         per_date:personal_task_date.text,
         per_status:personal_task_status,);
       setState(() {});
                            ...
                            child:  TextField(
                              onChanged: (value) {
                                _debouncer.run(() async {
                                  ...
                                  await personal_task_manager_onsearch(
                                    isReload: true,
...
//                          width: MediaQuery.of(context).size.width * 0.3),
  ...
  body_layout() {
    return SmartRefresher(
        ...
        onRefresh: () async {
          if (filter_flag == true) {
            final result = await personal_task_filter_manager(
                isReload: true,
              per_date:personal_task_date.text,
              per_status:personal_task_status,);
            print(result);
            ...
            final result = await personal_task_manager(isReload: true);
            if (result != null && result.isNotEmpty) {
              _refreshController.refreshCompleted();
            } else {
              _refreshController.refreshFailed();
            ...
            final result = await personal_task_manager_onsearch(
        ...
        onLoading: () async {
          if (filter_flag == true) {
            ...
            var result = await personal_task_manager();
            if (result != null && result.isNotEmpty) {
              _refreshController.loadComplete();
            } else {
              _refreshController.loadFailed();
            ...
            final result =
            await personal_task_manager_onsearch(word: searchedWord);
            print(result);
            if (result != null && result.isNotEmpty) {
              _refreshController.loadComplete();
            } else {
        ...
        my_nodata():task_manager_list.isNotEmpty
            ? ListView.builder(
            itemCount:
            task_manager_list == null ? 0 : task_manager_list.length,
                                          ...
                                          children: [
                                            ...
                                            Padding(
                                              ...
                                              child: Container(
                                                height: 15.0,
                                                child: Text(
                                                  task_manager_list[index].status,
                                                  maxLines: 1,
                                                  overflow: TextOverflow.ellipsis,
                                                  textAlign: TextAlign.left,
                                                  style: RowStyle_grey(10.0),
...
//                               alignment: Alignment.topCenter,
                                          ...
                                          child: LinearProgressIndicator(
                                            value: parseStatusindicatorvalue(task_manager_list[index].status),
                                            valueColor: parseStatusAnimationIntoColor(task_manager_list[index].status),
                                            backgroundColor:  parseStatusIntoOpacityColorandpercentage(task_manager_list[index].status),
                                          )
                                      ),
