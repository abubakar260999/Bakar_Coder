...
class notstarted extends StatefulWidget {
  @override
  _notstartedState createState() => _notstartedState();
}
...
class _notstartedState extends State<notstarted> {
  ...
  Widget showbar = view_appbar_txt("Not Started Task");

  final RefreshController _refreshController = RefreshController(initialRefresh: true);
  List<TaskManager> notstarted_task_manager_list = [];
  ...
  Future<List<TaskManager>> task_manager({bool isReload = false}) async {
    try {
      ...
      String url = http_root + "not_started.php?page_no=$current_page&size=20";
      ...
      if (response.statusCode == 200) {
        ...
        if (result != null && result.isNotEmpty) {
          notstarted_task_manager_list.addAll(result);
          current_page++;
        }
        setState(() {});
        return result;
...
//searched data
  Future<List<TaskManager>> task_manager_onsearch(
    ...
    try {
      ...
      String url = http_root + "not_started.php?page_no=$current_search_page&size=5&word=$word";
      final response = await http.get(url);
      List<TaskManager> result = [];
      if (response.statusCode == 200) {
        final parsed = json.decode(response.body).cast<Map<String, dynamic>>();
...
//                      setState(() {
//                        if (notstarted_task_manager_list.isNotEmpty) {
//                          notstarted_task_manager_list = [];
//                        }
//                        close = false;
//                      });
...
//              setState(() {});
//              showbar = view_appbar_txt("Not Started Task");
//            },
//          )
//        ],
//      ),
  ...
  body_layout() {
    return SmartRefresher(
        ...
        child: notstarted_task_manager_list != null
            ? ListView.builder(
            itemCount:
            notstarted_task_manager_list == null ? 0 : notstarted_task_manager_list.length,
            itemBuilder: (context, index) {
              print(notstarted_task_manager_list.length);
              var list = notstarted_task_manager_list[index];
...
//                                      ||list.name1.isNotEmpty
                                ?list.name1:"",
                            notstarted_task_manager_list[index].assignto.isEmpty
//                                      ||list.name2.isNotEmpty
                                ?empty_assign_to:notstarted_task_manager_list[index].assignto.cast<String>(),
                            list.priority!=null
...
//                                      ||list.status.isNotEmpty
                  ...
                  setState(() => notstarted_task_manager_list[index].status = updated_status);
                                          ...
                                          children: [
                                            ...
                                            Padding(
                                              ...
                                              child: Container(
                                                height: 15.0,
                                                child: Text(
                                                  notstarted_task_manager_list[index].status==null?"":notstarted_task_manager_list[index].status,
                                                  maxLines: 1,
                                                  overflow: TextOverflow.ellipsis,
                                                  textAlign: TextAlign.left,
                                                  style: RowStyle_grey(10.0),
...
//                               alignment: Alignment.topCenter,
                                          ...
                                          child: LinearProgressIndicator(
                                            value: parseStatusindicatorvalue(notstarted_task_manager_list[index].status==null?"":notstarted_task_manager_list[index].status),
                                            valueColor: parseStatusAnimationIntoColor(notstarted_task_manager_list[index].status==null?"":notstarted_task_manager_list[index].status),
                                            backgroundColor:  parseStatusIntoOpacityColorandpercentage(notstarted_task_manager_list[index].status==null?"":notstarted_task_manager_list[index].status),
                                          )
                                      ),
